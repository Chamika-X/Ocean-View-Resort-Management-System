...
<style>
    ...
    .room-card:hover { transform: translateY(-4px); box-shadow: 0 12px 24px rgba(0,0,0,0.1); }
...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 fixed left-0 top-0 h-full bg-gradient-to-b from-cyan-400 via-cyan-500 to-blue-600 text-white flex flex-col z-50 shadow-xl">
        ...
        <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
            <a href="dashboard" class="sidebar-link active flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-white">
                <i class="fas fa-th-large w-5 text-center"></i>
                <span>Dashboard</span>
            </a>
            <a href="room?action=list" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-white/80 hover:text-white">
                <i class="fas fa-bed w-5 text-center"></i>
                <span>Rooms & Suites</span>
            </a>
            <a href="reservation?action=list" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-white/80 hover:text-white">
                <i class="fas fa-calendar-check w-5 text-center"></i>
                <span>Reservations</span>
            </a>
            <a href="guest?action=list" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-white/80 hover:text-white">
                <i class="fas fa-user-friends w-5 text-center"></i>
                <span>Guest Profiles</span>
            </a>
            <c:if test="${sessionScope.role == 'admin'}">
                <a href="activity?action=list" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-white/80 hover:text-white">
                    <i class="fas fa-hiking w-5 text-center"></i>
                    <span>Activities</span>
                </a>
                <a href="user-management?action=list" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-white/80 hover:text-white">
                    <i class="fas fa-user-cog w-5 text-center"></i>
                    <span>User Management</span>
                </a>
            </c:if>
            <a href="dashboard?action=help" class="sidebar-link flex items-center space-x-3 px-4 py-3 rounded-lg text-sm font-medium text-white/80 hover:text-white">
